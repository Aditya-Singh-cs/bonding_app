...
class DatabaseService {
      ...
      await _db.runTransaction((transaction) async {
        ...
        if (!snapshot.exists) {
          transaction.set(userRef, {
            ...
            'walletBalanceInPaise': 0,    // Naya logic: Asli paise
            'isTrialUsed': false, 
            'role': 'user',       
  ...
  // }
  // database_service.dart mein replace karo
Future<void> decreaseBalance(int secondsSpent, {required int pricePerMin}) async {
  if (uid == null) return;
  DocumentReference userRef = _db.collection('users').doc(uid);
  try {
    await _db.runTransaction((transaction) async {
      ...
      if (snapshot.exists) {
        ...
        bool trialUsed = data['isTrialUsed'] ?? false;

        if (trial > 0 && !trialUsed) {
          // Trial Mode: Seconds se Seconds kato
          int newTrial = (trial - secondsSpent).clamp(0, 999999).toInt();
          transaction.update(userRef, {
            'trialSeconds': newTrial,
            'isTrialUsed': newTrial <= 0,
          });
        } else {
          // Precision Mode: Per-second billing logic
          // 1 minute ki cost paise mein = pricePerMin * 100
          // 1 second ki cost = (pricePerMin * 100) / 60
          double costPerSecond = (pricePerMin * 100) / 60;
          int totalPaiseToDeduct = (secondsSpent * costPerSecond).ceil();
          
          int newWallet = (walletInPaise - totalPaiseToDeduct).clamp(0, 9999999).toInt();
          transaction.update(userRef, {
            'walletBalanceInPaise': newWallet,
            'isTrialUsed': true,
          });
        }
      }
  ...
  Future<void> addBalance(int amountToAdd) async {
    ...
    await _db.collection('users').doc(uid).update({
      'walletBalanceInPaise': FieldValue.increment(amountToAdd),
    });
  }
